...
CI lives in `.github/workflows/release-publish.yml` (every platform, Microsoft Store included), plus `attach-store-msix.yml` and the read-only `store-submission-inspect.yml`. Multi-platform releases are triggered via `workflow_dispatch` or tag pushes `v*`.
