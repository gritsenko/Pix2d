...
CI lives in `.github/workflows/release-publish.yml`, `windows-store-msix.yml` and `attach-store-msix.yml`. Multi-platform releases are triggered via `workflow_dispatch` or tag pushes `v*`.
...
6. **Submit the MSIX to the Store.** The tag push already built `Pix2d_X.Y.Z.0_x64_arm64.msixbundle` via [`windows-store-msix.yml`](.github/workflows/windows-store-msix.yml); the same workflow can upload it to Partner Center through the [Microsoft Store Developer CLI](https://learn.microsoft.com/windows/apps/publish/msstore-dev-cli/github-actions) instead of doing it by hand. It is **manual-only and two-stage** on purpose: `gh workflow run "Windows Store MSIX Package" -f version=X.Y.Z -f publish_to_store=true` rebuilds the bundle and leaves a **draft** submission (package uploaded, nothing sent to certification) — check in Partner Center that *Packages* lists the new version, then re-run with `-f submit_for_certification=true` to commit it. Needs a `SELLER_ID` repo secret (Partner Center → Account settings → Identifiers) alongside the existing `TENANT_ID`/`CLIENT_ID`/`CLIENT_SECRET`, and the Entra app must hold the **Manager** role. Three constraints baked into the tooling: msstore refuses non-free products (Pix2d is free, so fine); **a submission created by the API must never be edited by hand in Partner Center** — doing so permanently blocks further API commits on it, and the only fix is to delete the submission; and **keep the submission page closed in Partner Center while the job runs** — a commit racing an open dashboard page fails with `CommitFailed`/`InvalidState`, which msstore mistranslates as "active validation errors which cannot be exposed via API". Recovery is a plain re-run (msstore deletes the failed pending submission and creates a new one). Note the Partner Center UI cannot show a `PendingUpload` package, so after a dry run (`publish_to_store` alone) the Packages section still reads *Unchanged* — that is expected, not a failure; verify the staging with the read-only [`store-submission-inspect.yml`](.github/workflows/store-submission-inspect.yml) probe, which prints `applicationPackages` and `statusDetails` straight from the API. The retired StoreBroker job failed silently here (submission created, old packages republished) because it neither declared the new package nor uploaded the ZIP layout the SAS url expects; see the comment block in the workflow before touching this.
