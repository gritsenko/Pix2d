...
- [ ] **Test projects**: unit tests for document model, undo/redo, serialization, exporters. Framework: xUnit/NUnit; move root-level `test*.cs` logic here.
  - [x] **Headless scenario harness (spike)**: [`Sources/Tools/Pix2d.ScenarioTests`](../Sources/Tools/Pix2d.ScenarioTests) boots the *real* DI graph without a window (`AppBuilder.Configure<HarnessApp>().UsePlatformDetect().SetupWithoutStarting()` for the AssetLoader/Dispatcher/FontManager, a `HeadlessBootstrapper : Pix2dBootstrapperDI` loading only Sprite+Drawing+Png with a no-op `IPlatformStuffService`), stands up a non-rendering `ViewPort` + `SKInput`, creates a project, then (1) enumerates every registered command as a coverage inventory, (2) executes the tool-activation commands, (3) drives a scripted stroke via `SKInput.SetPointer*` (world→viewport via `ViewPort.WorldToViewport`) and asserts the layer bitmap + undo/redo round-trip. Proves the model layer is fully drivable headlessly. Not in `Pix2d.slnx` (same policy as `Pix2d.FormatTests`); run manually pre-release. **Next:** grow the scripted scenarios (layers, frames, artboards, export), add a curated safe-execute pass over more commands (blind execution bypasses `EditContextType` — `ExecuteCommandAsync` only checks the `_canExecute` flag — so a full sweep needs a destructive/modal skiplist + fakes for file dialogs), and wire it into CI via `Avalonia.Headless` (the current `UsePlatformDetect` path needs a display-less platform for Linux CI).
