...
namespace Pix2d.Plugins.Sprite.ViewModels
{
    public class SpriteAnimationTimelineViewModel : Pix2dViewModelBase
    {
        ...
        public ObservableCollection<AnimationFrameViewModel> Frames { get; set; } =
        ...
        //public bool IsPanelVisible => GetService<IMenuController>()?.ShowTimeline ?? false;

        public bool IsPlaying => _editor?.IsPlaying ?? false;

        public ICommand AddFrameCommand => MapCommand(SpritePlugin.AnimationCommands.AddFrame, OpLogCmd);

        public ICommand DuplicateFrameCommand => MapCommand(SpritePlugin.AnimationCommands.DuplicateFrame, OpLogCmd);
        ...
        public ICommand TogglePlayCommand => MapCommand(SpritePlugin.AnimationCommands.TogglePlay, OpLogCmd);
        public ICommand StopCommand => MapCommand(SpritePlugin.AnimationCommands.Stop, OpLogCmd);
        public ICommand PrevFrameCommand => MapCommand(SpritePlugin.AnimationCommands.PrevFrame);
        public ICommand NextFrameCommand => MapCommand(SpritePlugin.AnimationCommands.NextFrame);

        public SpriteAnimationTimelineViewModel(IMessenger messenger, IAppState appState)
        {
            Messenger = messenger;
            AppState = appState;
        ...
        private void OpLogCmd() => OpLog("cmd");
        private void OpLog([CallerMemberName] string caller = null)
        {
            SessionLogger.OpLog("f" + FrameIndex + "/" + FrameCount, caller);
        }
