...
/// <summary>
/// Middle resize thumb on the left edge: changes the frame width (the left edge moves, the right edge
/// stays fixed). Vertical drag is ignored unless aspect lock is active, in which case the height follows
/// the width proportionally. Mirrors <see cref="LeftTopResizeThumbSingleNode"/> with the Y axis pinned.
/// </summary>
public class LeftResizeThumbSingleNode : ResizeThumbSingleNode
{
    ...
    protected override void SetNewBounds(SKSize initialSize, SKPoint delta, bool lockAspect)
    {
        ...
        // Right edge stays fixed; with aspect lock the height grows symmetrically around the vertical center.
