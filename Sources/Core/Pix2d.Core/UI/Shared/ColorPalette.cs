...
public class ColorPalette : ComponentBase
{
    ...
    public IList<SKColor> Colors
    {
        ...
        set
        {
            if (Equals(_colors, value)) return;

            if (_colors is INotifyCollectionChanged oldCollection)
            {
                oldCollection.CollectionChanged -= OnColorsCollectionChanged;
            }

            SetAndRaise(ColorsProperty, ref _colors, value);

            if (_colors is INotifyCollectionChanged newCollection)
            {
                newCollection.CollectionChanged += OnColorsCollectionChanged;
            }
    ...
    private void RebuildChildren()
    {
        ...
        if (Colors != null)
        {
            ...
            {
                _wrapPanel.Children.Add(CreateColorItem(color));
            }
        }

        if (CanAddColor)
        {
            _wrapPanel.Children.Add(CreateAddButton());
        }
    }
